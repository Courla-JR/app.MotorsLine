...
function StatusBadge({ status }: { status: DbMission["status"] }) {
  ...
  return (
    <span className="px-3 py-1 rounded-full bg-[#ffb4ab]/10 text-[#ffb4ab] text-[10px] font-bold uppercase tracking-wider">
      Annulée
    </span>
  );
}
...
export default function ClientMissionsPage() {
  ...
  // Fetch missions whenever clientId or filter changes
  useEffect(() => {
      ...
      <main className="pt-24 pb-32 px-4 max-w-lg mx-auto">
        ...
        <div className="grid gap-6">
          {missions.map((m) => (
            <div
              key={m.id}
              className={`relative group ${m.status === "terminee" || m.status === "annulee" ? "opacity-70" : ""}`}
            >
              <div className="relative bg-[#1c1b1b] rounded-2xl p-6 flex flex-col gap-4">
