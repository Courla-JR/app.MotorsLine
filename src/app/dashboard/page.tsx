...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Today's missions (en_cours or pickup today)
      setTodayMissions(
        list.filter(
          (m) =>
            m.status === "en_cours" ||
            (m.status === "a_faire" && isToday(m.pickup_date))
        )
      );

      // Upcoming (a_faire, pickup in the future, not today)
      setUpcomingMissions(
        list.filter(
          (m) =>
            m.status === "a_faire" &&
            !isToday(m.pickup_date) &&
            isFuture(m.pickup_date)
        ).slice(0, 3)
      );
    ...
    <div className="bg-[#0A0A0A] min-h-screen pb-24">
      ...
      <main className="max-w-md mx-auto px-6 pt-6">
        ...
        <section className="mb-10 overflow-x-auto flex gap-4 -mx-6 px-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {[
            { label: "Total missions", value: loading ? "—" : String(stats.total), unit: "" },
            { label: "En cours", value: loading ? "—" : String(stats.en_cours), unit: "" },
            { label: "À faire", value: loading ? "—" : String(stats.a_faire), unit: "" },
          ].map((stat) => (
            <div key={stat.label} className="min-w-[160px] bg-[#1A1A1A] p-5 rounded-lg flex flex-col justify-between h-32 border border-white/5 shrink-0">
              ...
              <span className="text-white text-3xl font-bold" style={{ fontFamily: "Inter, sans-serif" }}>
                {stat.value}
                {stat.unit && <span className="text-sm font-normal text-[#949493]"> {stat.unit}</span>}
              </span>
            </div>
          ))}
        </section>
